...
console.log('task 1a:', artists[3].bio);

/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 2 (not auto tested): 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 
(no function needed) 
There is a typo in your dataset 😱 The 9th artist, Vincent Van Gogh is currently Vincent Van Dough. Fix this issue and console.log() to check your work. */
artists[8].name = 'Vincent Van Gogh';
console.log('task2:', artists);
...
function listOfNames(array) {
  const copyofArtists = [...array];
  for(let i = 0; i < array.length; i++){
    copyofArtists[i] = copyofArtists[i].name
  }
...
function addArtist(array,name,year,genre,nationality,bio) {
  const newArtist = ({name: name, years: year, genre: genre, nationality: nationality, bio: bio});
  array.push(newArtist);
  return array;
 }
 console.log('task 6', addArtist(artists, 'Sydney Hopson', '1985-2022', 'Full Stack Developmennt', 'African American', 'I have a background in Music production and computer science. I am attending BloomTech to become a backend Developer.')); 
...
function lotsOfArt(array) {
  const copyofArtists = array.filter(i => i.paintings > 100);
...
function artistInfo(array,name){
  return array.find(i => i.name === name).bio;
}
...
function artistByCountry(array, nationality){
  return array.filter(i => i.nationality === nationality).map(i => i = i.name);
 }
